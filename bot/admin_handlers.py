...
async def admin_titles_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    message += "• `/reject <submission_id> [reason]` - Reject a title\n"
...
async def reject_title_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not context.args or len(context.args) < 1:
        await update.message.reply_text(
            ...
            "Example: `/reject TS-20251223120000-123456789 Title too vague`",
            parse_mode='Markdown'
        )
        return
...
async def admin_stats_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    message += f"• Creator requests: {req_stats['creator_requests']}\n"
    message += f"• Content requests: {req_stats['content_requests']}\n"
    message += f"• Total: {req_stats['total_requests']}\n\n"
