...
async def handle_callback_query(update: Update, context: ContextTypes.DEFAULT_TYPE, bot_instance) -> None:
    ...
    if data == "search_creator":
        ...
        await handle_set_filters(query, session)
    elif data == "help":
        ...
        await handle_filter_menu(query, session, data)
    elif data.startswith("set_filter_"):
        await handle_apply_filter(query, session, data)
...
async def handle_back_to_list(query, session) -> None:
    ...
    directory_text = format_directory_text(creator_name, content_directory, session.filters)
    keyboard = create_content_keyboard(content_directory['items'], page=session.current_page)
