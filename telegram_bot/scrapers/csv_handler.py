...
def _get_default_csv_path():
    """Get the default path to onlyfans_models.csv in shared directory"""
    base_dir = Path(__file__).parent.parent.parent
    return str(base_dir / 'shared' / 'data' / 'onlyfans_models.csv')

_DEFAULT_CSV_PATH = _get_default_csv_path()
...
def get_all_creators_from_csv(csv_path: str = None, max_results: Optional[int] = None) -> List[dict]:
    """
    Get all creators from CSV file.
    
    Args:
        csv_path: Path to the CSV file (defaults to shared/data/onlyfans_models.csv)
