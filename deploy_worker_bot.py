...
# Directory where the repository will be copied
...
def copy_repository() -> bool:
    """Copy the current repository to the deployment directory."""
    print_section("Copying Repository")
    ...
    return run_command(
        ["cp", "-r", str(current_dir), CLONE_DIRECTORY],
        f"Copying repository from {current_dir} to {CLONE_DIRECTORY}"
    )
...
def install_dependencies() -> bool:
    ...
    landing_server_dir = Path(CLONE_DIRECTORY) / "landing_server"
    ...
    requirements_file = landing_server_dir / "requirements.txt"
...
def create_env_file() -> bool:
    """Create .env file in landing_server directory."""
    print_section("Creating Environment Configuration")
    
    landing_server_dir = Path(CLONE_DIRECTORY) / "landing_server"
    env_file = landing_server_dir / ".env"
    ...
    try:
        ...
        with open(env_file, "w") as f:
            f.write("# FreeFans Landing Server Configuration\n")
...
def verify_shared_directory() -> bool:
    """Verify that landing_server directory is available."""
    print_section("Verifying Landing Server Directory")
    
    repo_path = Path(CLONE_DIRECTORY)
    landing_server_dir = repo_path / "landing_server"
    
    if landing_server_dir.exists():
        print_success(f"Landing server directory found at {landing_server_dir}")
        return True
    else:
        print_warning(f"Landing server directory not found at {landing_server_dir}")
        print_info("The landing_server directory is required for the server to function.")
...
def cleanup_disk() -> None:
    ...
    try:
        ...
        unnecessary_dirs = [
            Path(CLONE_DIRECTORY) / "telegram_bot",  # Not needed for landing server
            Path(CLONE_DIRECTORY) / "core",  # Duplicate in landing_server
            Path(CLONE_DIRECTORY) / "managers",  # Duplicate in landing_server
            Path(CLONE_DIRECTORY) / "scrapers",  # Duplicate in landing_server
            Path(CLONE_DIRECTORY) / "utilities",  # Not essential for landing server
...
def start_landing_server() -> bool:
    """Start the landing server using system Python."""
    print_section("Starting Landing Server")
    
    landing_server_dir = Path(CLONE_DIRECTORY) / "landing_server"
    
    print_info(f"Python executable: {PYTHON_EXECUTABLE}")
    print_info(f"Working directory: {landing_server_dir}")
    print_info("Starting landing server process...")
    print()
    
    try:
        subprocess.run(
            [PYTHON_EXECUTABLE, "main.py"],
            cwd=str(landing_server_dir),
            check=False
        )
        return True
    except Exception as e:
        print_error(f"Failed to start landing server: {e}")
        return False
...
def main() -> int:
    ...
    print_header("FreeFans Landing Server Deployment")
    
    print_info("This script will deploy the FreeFans landing server.")
    print_info("Configuration:")
    print(f"  Copy to: {CLONE_DIRECTORY}")
    ...
    print(f"  Landing server directory: {CLONE_DIRECTORY}/landing_server")
    print(f"  Configuration file: {CLONE_DIRECTORY}/landing_server/.env")
    print()
    
    print_info("Next steps:")
    print("  1. Verify configuration in .env file")
    print(f"  2. Ensure RabbitMQ is running at {RABBITMQ_URL}")
    print("  3. Run the landing server:")
    print(f"     {PYTHON_EXECUTABLE} {CLONE_DIRECTORY}/landing_server/main.py")
    print()
    
    # Ask if user wants to start the bot
    response = get_user_input(
        "Start the landing server now? (yes/no)",
        default="yes"
    )
    
    if response.lower() == "yes":
        print_header("Starting Landing Server")
        if start_landing_server():
            return 0
        else:
            return 1
